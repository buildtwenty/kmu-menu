...
**Web push (PWA):** `manifest.json` + `icons/` + `sw.js` (push → notification, click → focus/open `/kmu-menu/`; no caching). The header bell button (`#bellBtn`, icon-only: outline = not subscribed, filled+check = on, dimmed = blocked) and the drawer's "아침 알림 받기" button share one flow (`setupPush()`/`onPushClick()`/`updatePushBtn()` in `index.html`); the flow subscribes with the public VAPID key and POSTs the subscription to Supabase REST (`subscriptions` table: `id, endpoint(unique), keys jsonb, restaurants, keywords, created_at`; RLS lets anon insert only, so 409 = already subscribed). iOS Safari outside standalone gets a "홈 화면에 추가" guide sheet instead. When already subscribed, the bell opens the settings sheet (`openPushSettings()`): restaurant multi-select + keyword chips, saved by PATCHing the own row with an `X-Endpoint` header (RLS policy in `supabase/rls_update_policy.sql` matches that header against `endpoint`; PostgREST returns 204 even when RLS matched 0 rows, so the client checks `Content-Range` via `count=exact`). The last saved prefs are mirrored in localStorage (`kmu-push-prefs`) because anon cannot read its row. "알림 그만 받기" only unsubscribes in the browser; the row is cleaned up when the next send gets 410. `scripts/send_push.py` builds the day's summary from `menus.json` (ports `splitDish`), personalizes per row (`restaurants` → only those restaurants, falling back to the full summary if none of them has menus; `keywords` → substring match like calories, adds `🎯 오늘 '돈까스' 있어요 — 학생식당` lines on top; payloads cached per prefs combo), and sends via pywebpush; 404/410 rows are deleted. `--dry-run --subs-file rows.json` renders personalization locally. Secrets (`SUPABASE_SECRET_KEY`, `VAPID_PRIVATE_KEY` PEM) live only in GitHub Secrets and `vapid_private.pem` is gitignored — never put them in code. The Supabase URL, publishable key and VAPID public key are public values hardcoded in `index.html`/`send_push.py`. All PWA paths are absolute under `/kmu-menu/` (project page), so local testing needs the repo served under that prefix.
...
- 판촉 접두어 정리 필터 (이슈 #5에서 승격): 'New'·'★여름간식판매★' 같은 홍보 줄/접두어를
  메뉴명에서 제거 (`PROMO_ONLY_PAT` 줄 스킵 + `strip_promo_prefix()` 앞머리 제거).
  '판매'는 '철판매운오돈볶음'에 substring으로 걸리므로 토큰 앵커로 한정. 저장 데이터도 1회 정리
...
- 알림 2차 (2026-09-19): 구독 중 벨 → 알림 설정 시트(식당 다중 선택·키워드 칩·저장·그만 받기),
  send_push.py 개인화(식당 필터 + 🎯 키워드 줄). 설정 저장은 supabase/rls_update_policy.sql
  (X-Endpoint 헤더 기반 자기 행 update 정책)을 SQL Editor에서 실행해야 동작. '매칭된 날만 받기'는 미구현
### 다음 순서
