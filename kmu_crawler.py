...
# '추석연휴'·'설 연휴'처럼 명절명 뒤에 '연휴'가 붙은 단독 표기도 같은 취급(휴무 안내).
_HOLIDAY_EDGE = r"[\s*★☆♡♥◇※()\[\]<>·.\-]*"
_HOLIDAY_NAME = (
    r"(?:제헌절|광복절|개천절|한글날|현충일|삼일절|3\.?1절|어린이날|근로자의\s*날"
    r"|성탄절|크리스마스|석가탄신일|부처님\s*오신\s*날|신정|설날?|추석|한가위|명절|대체\s*공휴일)"
)
HOLIDAY_PAT = re.compile(
    rf"^{_HOLIDAY_EDGE}(?:{_HOLIDAY_NAME}(?:\s*연휴)?|연휴){_HOLIDAY_EDGE}$"
)

# 명절/연휴 인사 문장(예: '풍요로운 추석연휴 보내세요', '즐거운 명절 되세요').
# 메뉴명은 '~세요/~십시오/~길/~합니다'로 끝나지 않으므로, 줄 전체가 이런 인사 어미로
# 끝날 때만(뒤 장식·문장부호 허용) 버린다. 문장 중간의 단어(추석·명절 등)는 보지 않아
# '추석특식 송편'·'명절맞이 갈비찜' 같은 진짜 메뉴 줄은 걸리지 않는다.
GREETING_PAT = re.compile(
    r"^.*?(?:보내세요|되세요|되십시오|보내십시오|보내시길|되시길|기원합니다|축하합니다|감사합니다)"
    r"[\s.!~♡♥★☆※)\]>]*$"
)
...
def parse_cell(lines: list, default_meal, hours_sink=None):
    ...
    for line in lines:
        ...
        if NOTICE_PAT.search(line):
            continue
        # 공휴일/절기 단독 표기(예: '제헌절', '추석연휴')는 메뉴가 아니므로 스킵
        if HOLIDAY_PAT.match(line):
            continue
        # 명절 인사 문장(예: '풍요로운 추석연휴 보내세요')도 스킵
        if GREETING_PAT.match(line):
            continue
